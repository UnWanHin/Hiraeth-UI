...
SERVICE_FILE="$UNIT_DIR/studio-portal-benchmark.service"
TIMER_FILE="$UNIT_DIR/studio-portal-benchmark.timer"
...
[Unit]
Description=Refresh Studio Portal YABS benchmark data
...
cat > "$TIMER_FILE" <<UNIT
[Unit]
Description=Refresh Studio Portal YABS benchmark data on a safe interval
...
Persistent=true
Unit=studio-portal-benchmark.service
...
systemctl --user enable --now studio-portal-benchmark.timer
systemctl --user list-timers studio-portal-benchmark.timer --no-pager
